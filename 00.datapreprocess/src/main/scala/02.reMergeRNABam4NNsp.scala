...
import MetaData.{toStringOfAllenIdLabel, isNeu}
import SZUtils.writeStrings2File

object MergeRNABam4NNsp {
  ...
  extension (a: PairedTagBarcodeMeta) {
    def spTostr: String = {
      if (a.annot.sp.isDefined) {
        toStringOfAllenIdLabel(a.annot.sp.get)
      } else {
        ""
      }
    }
  ...
  def main(args: Array[String]) = {
    ...
    val tasks = scG2blstfnm.keys.map((sp, rep) => new MergeRNABamTask(sp, rep))
    tasks.par.foreach { t =>
      {
        t.run()
