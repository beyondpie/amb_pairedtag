#' @title filterNULLfromList
#' @description Filter NULL from a list
#' @param l list
#' @return list or NULL
#' @export
...
#' Simply outout a vector to a file.
#' @export
write_one_column <- function(vec, f) {
  write.table(
    vec, file = f, quote = FALSE, row.names = FALSE, col.names = FALSE)
}

#' Save a matrix into hdf5 format.
#' @description
#' In the hdf5 file, a group X is created, which
#' has three fileds, mat, colnames, rownames
#' In Python h5py package, we can access the mat by
#' `f['X']['mat'][()]`, where f is a hdf5 file handle.
#' NOTE: hdf5r will transpose the mat
#' https://github.com/hhoeflin/hdf5r/issues/81
#' @param mat matrix, group by feature
#' @param peaks colnames of the matrix
#' @param clusters rownames of the matrix
#' @param outf output filename
#' @return NULL/Nothing
#' @export
saveCPM2h5 <- function(mat, peaks, clusters, outf) {
  conn <- hdf5r::H5File$new(outf, mode = "w")
  data.grp <- conn$create_group("X")
  data.grp[["mat"]] <- mat
  data.grp[["colnames"]] <- peaks
  data.grp[["rownames"]] <- clusters
  conn$close_all()
  message("saved cpm to file: ", outf)
}
...
#' @title capMat
#' @description Cap the values of a matrix
#' @param mat A matrix
#' @param lowQ Lower quantile
#' @param upQ Upper quantile
#' @export
capMat <- function(mat, lowQ = 0.005, upQ = 0.995) {
  library(Matrix)
  lowQv <- quantile(mat, probs = lowQ)
  upQv <- quantile(mat, probs = upQ)
  mat[mat < lowQv] <- lowQv
  mat[mat > upQv] <- upQv
  mat
}
