...
#' @param metric Distance metric; can be one of
...
AnnoyNN <- function(data,
                    ...
                    index = NULL) {
  idx <- seuratOR(index, AnnoyBuildIndex(
    data = data,
    metric = metric,
    n.trees = n.trees
  ))
...
#' can be one of "euclidean", "cosine", "manhattan", "hamming".
#' @param n.trees More trees gives higher precision when querying
...
#
#' @export
AnnoySearch <- function(index, query, k,
