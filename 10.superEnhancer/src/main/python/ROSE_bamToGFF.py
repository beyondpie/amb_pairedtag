...
def mapBamToGFF(
    bamFile, gff, sense="both", extension=200, floor=0, rpm=False, matrix=None
):
    ...
    newGFF.append(
        ...
        + [
            ...
            for n in range(1, int(matrix) + 1, 1)
        ]
    )
    ...
    for line in gff:
        ...
        gffLocus = ROSE_utils.Locus(
            line[0], int(line[3]), int(line[4]), line[6], line[1]
        )
        # print line[0]
        # sys.exit()
        ...
        if gffLocus.sense() == "+" or gffLocus.sense == ".":
            senseReads = [
                x for x in extendedReads if x.sense() == "+" or x.sense() == "."
            ]
        ...
        if (
            ...
            while n < nBins:
                ...
                binDen = (
                    float(sum([senseHash[x] + antiHash[x] for x in binKeys])) / binSize
                )
