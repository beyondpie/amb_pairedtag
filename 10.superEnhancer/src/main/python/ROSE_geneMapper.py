...
def main():
    ...
    cwd = os.path.join(gv.pt_projd, "10.superEnhancer",
                       "src/main/resource")
    genomeDict = {
        'HG18': '%s/hg18_refseq.ucsc' % (cwd),
        'MM9': '%s/mm9_refseq.ucsc' % (cwd),
        'HG19': '%s/hg19_refseq.ucsc' % (cwd),
        'HG38': '%s/hg38_refseq.ucsc' % (cwd),
        'MM8': '%s/mm8_refseq.ucsc' % (cwd),
        'MM10': '%s/mm10_refseq.ucsc' % (cwd),
    }
