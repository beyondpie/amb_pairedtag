...
object RunSuperSilencer {
  val projd: String = TSCCMeta.projd
  val h: String = "H3K27me3"
  ...
  val outd: String = s"${workd}/out/ROSE_${h}"
  val logd: String = s"${workd}/log/ROSE_${h}"

  val python: String = "/home/szu/miniforge3/envs/sa2/bin/python"
  val rscript: String = "/home/szu/miniforge3/envs/r/bin/Rscript"
  ...
  class SubclassROSE(
    ...
    val peakGFF = s"${outd}/${sc}/${sc}.${h}.peak.gff"
  ...
  def main(args: Array[String]) = {
    ...
    val scs: List[String] =
      os.list(os.Path(bamd))
        .map(d => d.baseName)
        .filter(d => os.exists(os.Path(s"${bamd}/${d}/${h}.srt.bam")))
        .filter(
          d => os.exists(os.Path(s"${peakd}/${d}-${h}.bedtoolmerge.peak")))
        .toList

    // val scs = List("326_OPC_NN", "327_Oligo_NN")

    val sc2bam =
      scs.map(sc => (sc, os.Path(s"${bamd}/${sc}/${h}.srt.bam"))).toMap

    val sc2peak =
      scs.map(sc => (
        sc, os.Path(s"${peakd}/${sc}-${h}.bedtoolmerge.peak"))).toMap
    ...
    // val stitch = 12500
    // val tssWindow = 2000
    // val bam = s"${bamd}/${sc}/H3K27me3.srt.bam"
    // val peakfnm = s"${peakd}/${sc}-H3K27me3.bedtoolmerge.peak"
    ...
    //  runROSE
    val scspar = scs.par
    scspar.tasksupport = new ForkJoinTaskSupport(
