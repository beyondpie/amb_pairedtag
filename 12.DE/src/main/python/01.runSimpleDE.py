...
def getDE(
    ...
    g1_pct = pd.Series(
        data=np.asarray(np.sum(ann[g1_barcodes].X > 0, axis=0)).squeeze() / n_g1,
        index=ann.var_names,
    )
    fea_by_pct = ann.var_names[g1_pct >= min_pct]
    print(f"{(fea_by_pct).size} features left after min_pct as {min_pct}.")
    g2_pct = pd.Series(
        data=np.asarray(np.sum(ann[g2_barcodes].X > 0, axis=0)).squeeze() / n_g2,
        index=ann.var_names,
    )
    # filter features by min_log2fc
    g1_cpm = getPseudoBulkCPM(ann[g1_barcodes])
    g2_cpm = getPseudoBulkCPM(ann[g2_barcodes])
    log2_fc = pd.Series(np.log2((g1_cpm + 1e-9) / (g2_cpm + 1e-9)), index=ann.var_names)
    fea_by_fc = ann.var_names[log2_fc >= min_log2fc]
    fea = fea_by_pct.intersection(fea_by_fc)
    print(f"{fea.size} features left after min_log2fc as {min_log2fc}.")
    ann1 = ann[pd.concat([g1_barcodes, g2_barcodes]),].copy()
    print("Normalize ann: CP100K per cell.")
    sc.pp.normalize_total(ann1, target_sum=1e5, inplace=True)
    ann1 = ann1[:, fea]
    sc.pp.log1p(ann1, copy=False)
    # run wilcoxon
    print(f"Run {method} test on group: {g1}.")
    ...
    if len(outfnm) > 1:
        ...
        rr.to_csv(outfnm, header=True, index=False, sep="\t")
    return rr
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="DE",
        description="Differential peak analysis by Wilcoxon rank sum test / t-test",
    )
    ...
    parser.add_argument("--method", type=str, default="wilcoxon", help="output of results")
    args = parser.parse_args()

    ann: ad.AnnData = ad.read_h5ad(args.annfnm)
    r = getDE(
        ann,
        g1=args.g1,
