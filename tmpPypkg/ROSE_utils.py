...
def regionStitching(inputGFF, stitchWindow, tssWindow, annotFile, removeTSS=True):
    ...
    boundCollection = gffToLocusCollection(inputGFF)
    ...
    if removeTSS:
        ...
        for geneID in list(startDict.keys()):
            tssLoci.append(makeTSSLocus(geneID, startDict, tssWindow, tssWindow))
    ...
    if removeTSS:
        ...
        tssLoci = []
        for geneID in list(startDict.keys()):
            tssLoci.append(makeTSSLocus(geneID, startDict, 50, 50))
        ...
        for stitchedLocus in stitchedCollection.getLoci():
            ...
            tssNames = [
                startDict[tssLocus.ID()]["name"] for tssLocus in overlappingTSSLoci
            ]
            tssNames = uniquify(tssNames)
            if len(tssNames) > 2:
...
def mapCollection(
    stitchedCollection, referenceCollection, bamFileList,
    mappedFolder, output, refName) -> None:
    """
    ...
    lenOrder = order(lociLenList, decreasing=True)
    ticker = 0
    for i in lenOrder:
        ticker += 1
        if ticker % 1000 == 0:
    ...
    for bamFile in bamFileList:
        ...
        mappedGFF = parseTable(
            "%s%s_%s_MAPPED.gff" % (mappedFolder, refName, bamFileName), "\t"
        )
    ...
    unParseTable(locusTable, output, "\t")
    return None
