...
def compute_metrics(
    ...
    if ndim != 2:
        raise RuntimeError(f"partitions should have 2 instead of {ndim} dims.")
    # * to unsigned int64
    ...
    pac: float = cal_PAC(consensus, u1=u1, u2=u2)
    return (disp, pac)
...
def cal_dispersion(consensus) -> float:
    ...
    """
    n = consensus.shape[1]
    corr_disp = np.sum(4 * np.square(consensus - 0.5), dtype=np.float64) / (
        np.square(n)
    )
    return corr_disp
...
def init_LeidenSum_from_file(
    from_dir: str, cll: str, cid: str, prefix: str = "sa2_clustering"
) -> LeidenSum:
...
def draw_umap(
    ...
    if t.nsample > scf.nsample:
        ...
        index = np.random.choice(range(t.nsample), size=scf.nsample, replace=False)
    x = t.umap[index, 0]
    y = t.umap[index, 1]
    for i in range(nrow):
        for j in range(ncol):
            ...
            if len(colors) >= len(ulabel_):
                ...
                colormap = matplotlib.colormaps[scf.palette].resampled(len(ulabel_))
            sc = ax.scatter(
                x=x,
                y=y,
                c=label,
            ...
            ax.legend(handles=sc.legend_elements()[0], labels=ulabel_.tolist())
...
def run_leiden(
    ...
    """
    Run leiden under a resolution with modualirty method with SnapATAC2.
    """
    # breakpoint()
    leidens = np.ndarray(shape=(sa2ann.shape[0], times_r), dtype="int")
    if sa2ann.shape[0] <= nsample_sil:
        if logger is not None:
            logger.info("Get silhouette score with all samples.")
        else:
            print("Get silhouette score with all samples.")
    else:
        if logger is not None:
            logger.info(
                f"Get silhouette score with {times_sil} times sampling.")
        else:
            print(f"Get silhouette score with {times_sil} times sampling.")
    
    nseed_sil: int = 1 if sa2ann.shape[0] <= nsample_sil else times_sil
    sils = np.ndarray(shape=(nseed_sil, times_r), dtype=float)
    for i in range(times_r):
        if logger is not None:
            logger.info(f"Run {i}th/{times_r} leiden.")
        else:
            print(f"Run {i}th/{times_r} leiden.")
        ...
        if np.unique(leidens[:, i]).shape[0] < 2:
            if logger is not None:
                ...
                print("No more clusters in current clustering.")
                print("Then set silhouette as 0.0.")
            sils[:, i] = 0.0
            continue
        if logger is not None:
            logger.info("Run silhouette.")
        else:
            print("Run silhouette.")
        for j in range(nseed_sil):
            sils[j, i] = silhouette_score(
                    X=sa2ann.obsm["X_spectral"],
                    labels=leidens[:, i],
                    sample_size=nsample_sil,
                    random_state=j)
    r1 = pd.DataFrame(
        data=leidens,
        index=sa2ann.obs_names,
        columns=[f"r{r:.1f}_seed{j}" for j in range(times_r)])
    r2 = pd.DataFrame(
        data=sils, index=[f"seed{j}" for j in range(nseed_sil)],
        columns=r1.columns)
    r1.insert(0, "barcode", r1.index)
    return (r1, r2)
