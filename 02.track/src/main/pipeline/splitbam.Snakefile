...
detailed_bamdirs = [os.path.join(bamdir, f"{r}_{h}_{s}")
                    for r in regions
                    for h in hms
                    for s in sexs]
...
rule all:
    input:
        expand("{f}/index_{d}_cell_bams.done", f = flagdir, d = sublib_DNAs),
        expand("{f}/split_{d}_cell_bams.done", f = flagdir, d = sublib_DNAs)
...
rule split_bam:
    ...
    threads: nthread
    script:
       f"{work_dir}/src/main/python/02.splitbam.py" 
